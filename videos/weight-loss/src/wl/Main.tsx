...
const bgmVolume = (f: number) => {
  ...
  if (f > WL_TOTAL - 48) return ((WL_TOTAL - f) / 48) * 0.3;
  return 0.3;
};

export const WlMain: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#0b4f73' }}>
      ...
      {bgm ? (
        <Sequence from={0} durationInFrames={WL_TOTAL}>
      ...
      {/* 六个场景 */}
      <Sequence from={WL_SHOTS.open.from} durationInFrames={WL_SHOTS.open.duration}>
        <SceneOpen />
      </Sequence>
      <Sequence from={WL_SHOTS.r1.from} durationInFrames={WL_SHOTS.r1.duration}>
        <SceneStarve />
      </Sequence>
      <Sequence from={WL_SHOTS.r2.from} durationInFrames={WL_SHOTS.r2.duration}>
        <SceneNight />
      </Sequence>
      <Sequence from={WL_SHOTS.r3.from} durationInFrames={WL_SHOTS.r3.duration}>
        <SceneCardio />
      </Sequence>
      <Sequence from={WL_SHOTS.r4.from} durationInFrames={WL_SHOTS.r4.duration}>
        <SceneScale />
      </Sequence>
      <Sequence from={WL_SHOTS.outro.from} durationInFrames={WL_SHOTS.outro.duration}>
        <SceneOutro />
      </Sequence>
      ...
      {/* 水泡闪光转场（跨骑硬切） */}
      {[WL_SHOTS.r1.from, WL_SHOTS.r2.from, WL_SHOTS.r3.from, WL_SHOTS.r4.from, WL_SHOTS.outro.from].map(
        (cut) => (
